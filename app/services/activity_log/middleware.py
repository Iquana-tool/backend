...
#: Never instrumented: the activity log endpoints themselves (a flush would record
#: its own arrival, forever), and the noisy liveness paths.
_SKIP_PREFIXES = ("/activity-log", "/docs", "/redoc", "/openapi.json", "/status")
...
class ActivityLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # carry neither the bearer token nor `X-Activity-Session` -- a preflight
        ...
        if not get_config().captures(ActivityComponent.API):
            return await call_next(request)
    ...
    def _record(self, request: Request, status_code: int, elapsed: float) -> None:
        try:
            ...
            logger.debug("Activity log middleware failed to record a request.",
                         exc_info=True)
