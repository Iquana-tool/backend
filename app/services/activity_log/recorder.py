...
Activity log must never be the reason an annotation request is slow, so nothing on
...
from app.services.activity_log.config import ActivityComponent, get_config
...
def truncate_payload(payload: dict[str, Any] | None) -> str | None:
    ...
    try:
        ...
        logger.warning("Activity log payload was not JSON-encodable; storing a marker.")
...
class ActivityRecorder:
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._drain_forever, name="activity-log-drain", daemon=True)
            self._thread.start()
        logger.info("Activity log recorder started.")
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        if self._dropped:
            logger.warning("Activity log dropped %d event(s) due to queue overflow.",
                           self._dropped)
        logger.info("Activity log recorder stopped.")
    ...
    def _enqueue(self, row: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self._dropped % 100 == 1:
                logger.warning("Activity log queue full; dropped %d event(s) so far.",
                               self._dropped)
            return False
...
def _write_rows_individually(session, rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        try:
            ...
            session.rollback()
    if stored != len(rows):
        logger.debug("Activity log stored %d/%d event(s); the rest were duplicates or invalid.",
                     stored, len(rows))
