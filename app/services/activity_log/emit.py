...
def emit_annotation(event_type: str, **kwargs: Any) -> None:
    ...
    recorder.record(ActivityComponent.ANNOTATION, event_type, **kwargs)


def emit_navigation(event_type: str, **kwargs: Any) -> None:
    """Session lifecycle events the server is the authority on (login, logout)."""
    recorder.record(ActivityComponent.NAVIGATION, event_type, **kwargs)


_EMITTERS = {
    ActivityComponent.API: emit_api,
    ActivityComponent.AI: emit_ai,
    ActivityComponent.ANNOTATION: emit_annotation,
    ActivityComponent.NAVIGATION: emit_navigation,
}


@contextmanager
def track_duration(component: ActivityComponent, event_type: str, **kwargs: Any):
    ...
    inference is as visible in the study data as a successful one::
        ...
        with track_duration(ActivityComponent.AI, "ai.prompted.invoke",
