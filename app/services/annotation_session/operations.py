...
from app.services.activity_log.emit import track_duration
...
async def run_semantic_segmentation(
    ...
    with track_duration(ActivityComponent.AI, "ai.semantic.invoke",
                        username=str(user_id), session_id=session_id) as span:
...
async def run_instance_segmentation(
    ...
    with track_duration(ActivityComponent.AI, "ai.instance.invoke",
                        username=str(user_id), session_id=session_id) as span:
        span["model"] = model_registry_key
        response = await service.inference(request)
...
async def run_suggestion_segmentation(
    ...
    with track_duration(ActivityComponent.AI, "ai.suggestion.invoke",
                        username=str(user_id), session_id=session_id) as span:
        span["model"] = model_key
