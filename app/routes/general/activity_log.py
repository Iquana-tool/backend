"""Activity log ingest, runtime configuration and study-data export.
...
from app.services.activity_log.recorder import truncate_payload, recorder
from app.services.activity_log.schemas import (
    ActivityBatch,
    ActivityLogConfigUpdate,
...
async def optional_user(request: Request,
    ...
    Unlike `get_current_user` this never raises: an expired token on an activity log
...
async def read_config(db: Session = Depends(get_session)):
    ...
    """
    return {"success": True, "config": activity_log_config.get_config(db).as_response()}


@router.put("/config")
async def update_config(
        update: ActivityLogConfigUpdate,
        db: Session = Depends(get_session),
        user: AuthenticatedUser = Depends(require_global(Permission.ACTIVITY_LOG_MANAGE)),
):
    """Flip study logging or individual components without a redeploy."""
    current = activity_log_config.get_config(db)
    if not current.enabled:
        ...
        raise HTTPException(
            ...
            detail="Activity log is disabled for this deployment (USER_EVENTS_ENABLED).",
        )

    components = (frozenset(update.components)
                  if update.components is not None else None)
    resolved = activity_log_config.save_config(
        db,
        capture_enabled=update.capture_enabled,
        components=components,
        idle_threshold_ms=update.idle_threshold_ms,
        updated_by=user.username,
    )
    if resolved.capture_enabled:
        recorder.start()
    logger.info("Activity log config updated by %s: study=%s components=%s idle_ms=%d",
                user.username, resolved.capture_enabled,
                activity_log_config.format_components(resolved.components),
                resolved.idle_threshold_ms)
    return {"success": True, "config": resolved.as_response()}


@router.post("/events", response_model=ActivityIngestResponse)
async def ingest_events(
    ...
    return ActivityIngestResponse(accepted=accepted, dropped=dropped)
...
async def purge_events(
    ...
    Takes the same filters as ``GET /activity-log/events`` and ``/export`` (via the
    ...
    db.commit()
    logger.info("%s purged %d activity log event(s).", user.username, deleted)
